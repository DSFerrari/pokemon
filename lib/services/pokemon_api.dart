...
final res = await http.get(Uri.parse(url));
    if (res.statusCode == 200) {
      final listResponse = PokemonListApiResponse.fromJson(jsonDecode(res.body),);
    completer.complete(listResponse);
    }
       else{
        completer.completeError(Exception('Failed to fetch Pokemon list (statusCode: ${res.statusCode})'
        ),
        );
      }
      }catch(e){
